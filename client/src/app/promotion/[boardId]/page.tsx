...
import { apiSSRFetchPromotionBoard } from "@/apis/ssr";
...
export const generateMetadata = async ({ params: { boardId } }: Props): Promise<Metadata> => {
  const initialData = await apiSSRFetchPromotionBoard({ promotionBoardId: boardId });

  return getMetadata({
    title: initialData.title,
    description: initialData.content.replace(/<[^>]*>?/g, "") || "존재하지 않는 홍보 게시글입니다.",
