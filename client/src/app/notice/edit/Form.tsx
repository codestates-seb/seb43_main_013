...
// api
import { apiUpdateNoticeBoard } from "@/apis";
...
/** 2023/05/20 - 공지사항 게시글 수정 form 컴포넌트 - by leekoby */
const Form: React.FC<Props> = ({ boardId }) => {
  ...
  /** 2023/05/20 - 내용 불러오기 - by leekoby */
  const { data, isLoading } = useFetchNoticeBoard({ noticeId: boardId });

  /** 2023/05/20 - 작성된 내용 채워넣기 - by leekoby */
  useEffect(() => {
    if (!data) return;
    setContent(data.content);
  }, [data]);

  /** 2023/05/20 - 공지사항 게시글 수정 - by leekoby */
  const onSubmit: React.FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    try {
      ...
      await apiUpdateNoticeBoard({
        noticeId: boardId,
        title,
        content,
      });

      queryClient.invalidateQueries(["noticeBoard", boardId + ""]);
